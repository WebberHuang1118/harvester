...
func (se *SnapshotEngine) Create(
	vmb *harvesterv1.VirtualMachineBackup,
	volIndex int,
	vscMap map[string]snapshotv1.VolumeSnapshotClass,
) error {
	logrus.Infof("SnapshotEngine Create called for VMBackup %s/%s volume index %d",
	...
	if vs == nil {
		vs, err = se.ensureVolumeSnapshotExists(vmb, vb, vscMap)
		if err != nil {
			return err
		}
	}
...
func (se *SnapshotEngine) ensureVolumeSnapshotExists(
	...
	vsc := vscMap[se.vmbo.GetVolBackupCSIDriver(vb)]
	ownerRef := se.vsHelper.BuildOwnerReference(vmb)

	return se.vsHelper.CreateVolumeSnapshotFromPVC(vmb, vb, &vsc, ownerRef)
}
