...
func NewVolumeSnapshotHelper(
	...
	return &VolumeSnapshotHelper{
		vsCache:         vsCache,
		vsClient:        vsClient,
		vsContentCache:  vsContentCache,
		vsContentClient: vsContentClient,
		vmbo:            vmbo,
		pvcCache:        pvcCache,
		scCache:         scCache,
	}
...
// ForceDeleteVolumeSnapshot removes finalizers and deletes a VolumeSnapshot
func (h *VolumeSnapshotHelper) ForceDeleteVolumeSnapshot(namespace, name string) error {
	vs, err := h.vsCache.Get(namespace, name)
	if apierrors.IsNotFound(err) {
		return nil
	}
	...
	vsCopy := vs.DeepCopy()
	vsCopy.Finalizers = []string{}

	logrus.WithFields(logrus.Fields{
		"namespace": namespace,
		"name":      name,
	}).Debug("removing finalizers from VolumeSnapshot")
	...
	logrus.WithFields(logrus.Fields{
		"namespace": namespace,
		"name":      name,
	}).Debug("deleting VolumeSnapshot")

	err = h.vsClient.Delete(namespace, name, metav1.NewDeleteOptions(0))
	if apierrors.IsNotFound(err) {
		return nil
	}
	if err != nil {
...
// ForceDeleteVolumeSnapshotContent removes finalizers and deletes a VolumeSnapshotContent
func (h *VolumeSnapshotHelper) ForceDeleteVolumeSnapshotContent(name string) error {
	vsContent, err := h.vsContentCache.Get(name)
	if apierrors.IsNotFound(err) {
		return nil
	}
	if err != nil {
	...
	vsContentCopy := vsContent.DeepCopy()
	vsContentCopy.Finalizers = []string{}

	logrus.WithField("name", name).Debug("removing finalizers from VolumeSnapshotContent")

	if _, err := h.vsContentClient.Update(vsContentCopy); err != nil {
		return fmt.Errorf("failed to remove finalizers: %w", err)
	}

	logrus.WithField("name", name).Debug("deleting VolumeSnapshotContent")

	err = h.vsContentClient.Delete(name, metav1.NewDeleteOptions(0))
	if apierrors.IsNotFound(err) {
		return nil
	}
	if err != nil {
...
func (h *VolumeSnapshotHelper) CreateVolumeSnapshotFromPVC(
	...
	vs := &snapshotv1.VolumeSnapshot{
		ObjectMeta: metav1.ObjectMeta{
			Name:            *h.vmbo.GetVolBackupName(vb),
			Namespace:       h.vmbo.GetNamespace(vmb),
		...
		Spec: snapshotv1.VolumeSnapshotSpec{
			Source: snapshotv1.VolumeSnapshotSource{
				PersistentVolumeClaimName: &vb.PersistentVolumeClaim.ObjectMeta.Name,
			},
