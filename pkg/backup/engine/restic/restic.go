...
func (re *ResticEngine) UpdateProgress(vb *harvesterv1.VolumeBackup) (int64, error) {
	...
	// Try to read the latest IO_PROGRESS line emitted by the running job's pod.
	// On any failure (pod not started yet, no progress emitted yet, transient
	// API error) fall back to the stored progress so we don't regress the value.
	jobName, jobNamespace, ok := re.jobLocator(vb)
	if !ok {
		return int64(re.vmbo.GetVolBackupProgress(vb)), nil
	}
	pct, err := jobprogress.FetchProgressPercent(context.Background(), re.clientset, jobNamespace, jobName)
	if err != nil {
		return int64(re.vmbo.GetVolBackupProgress(vb)), nil
	}
...
func (re *ResticEngine) createBackupJob(vmb *harvesterv1.VirtualMachineBackup,
	...
	repository, err := resticutil.Repository(target)
	if err != nil {
		return err
	}
			...
			Template: corev1.PodTemplateSpec{
				Spec: corev1.PodSpec{
					...
					Containers: []corev1.Container{{
						Name:    "backup",
						Image:   image,
						Env:     resticutil.Env(secretName, repository),
						...
						Args: []string{fmt.Sprintf(
							...
							// we initialize. Subsequent backups skip init via the short-circuit.
							...
							"set -eo pipefail; "+
								...
								"/usr/bin/harvester io-mode -device /dev/%s -mode=read | restic -q backup --stdin --stdin-filename %s --tag=%s,%s,%s",
							clonePVCName,
							pvName,
			...
			Template: corev1.PodTemplateSpec{
				Spec: corev1.PodSpec{
					...
					Containers: []corev1.Container{{
						...
						Env:     resticutil.Env(secretName, repository),
						Command: []string{"/bin/sh", "-c"},
						// Two --tag flags = AND. If the repo doesn't exist (backup never
						// pushed), short-circuit to success — nothing to forget.
						Args: []string{fmt.Sprintf(
							...
							"restic snapshots > /dev/null 2>&1 || exit 0; "+
								"restic forget --tag %s --tag %s --tag %s --unsafe-allow-remove-all --prune",
							resticutil.NamespaceTag(namespace),
							resticutil.VMBackupTag(re.vmbo.GetName(vmb)),
