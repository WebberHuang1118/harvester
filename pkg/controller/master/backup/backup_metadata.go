...
func (h *MetadataHandler) markVMBackupNotReady(vmb, vmbCpy *harvesterv1.VirtualMachineBackup, reason string) error {
	...
	vmbCpy = h.vmbo.UpdateChangeToNonReadyNoCommit(vmbCpy, reason)
...
func (h *MetadataHandler) checkVolumeInBackupTarget(
	...
	volumeName := h.vmbo.GetVolBackupVolumeName(vb)
	lhBackupName := h.vmbo.GetVolBackupLHBackupName(vb)
	...
	if volumes[volumeName] == nil {
		return h.markVolumeNotReady(
			...
			fmt.Sprintf("cannot find volume %s in the backup target", volumeName),
			logrus.Fields{"volume": volumeName},
		)
	}

	if volumes[volumeName].Backups[*lhBackupName] == nil {
		return h.markVolumeNotReady(
			vmb, vb,
