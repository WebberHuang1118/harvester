...
func (h *Handler) updateBackupProgress(volumeBackup *harvesterv1.VolumeBackup) error {
	defer func() {
		logrus.Infof("volumeBackup %v backup progress %v", *volumeBackup.Name, volumeBackup.BackupProgress)
	}()
...
func (h *Handler) updateConditions(vmBackup *harvesterv1.VirtualMachineBackup) error {
	...
	if volumeSizeSum != 0 {
		...
		logrus.Infof("vmbackup %s current progress %v", vmBackupCpy.Name, progressWeightSum/volumeSizeSum)
	}
