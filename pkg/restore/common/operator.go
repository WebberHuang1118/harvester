...
func (vmro *vmrestoreOperator) DeleteOldPVCs(vmr *harvesterv1.VirtualMachineRestore, vm *kubevirtv1.VirtualMachine) error {
	...
	if len(deletedVolumes) == 0 {
		logrus.Debugf("no deleted volumes to clean up for restore %s/%s", vmr.Namespace, vmr.Name)
		return nil
	}
	...
	for _, volume := range vm.Spec.Template.Spec.Volumes {
		if err := vmro.deletePVCIfNeeded(vm, vmr, &volume, deletedVolumes, &deletedCount, &errs); err != nil {
			// Error already added to errs slice
			continue
		}
	}

	if len(errs) > 0 {
		logrus.Errorf("deleted %d PVCs with %d errors for restore %s/%s", deletedCount, len(errs), vmr.Namespace, vmr.Name)
		return fmt.Errorf("failed to delete %d PVC(s): %v", len(errs), errs)
	}

	if deletedCount > 0 {
		logrus.Infof("successfully deleted %d old PVC(s) for restore %s/%s", deletedCount, vmr.Namespace, vmr.Name)
	}

	return nil
}

// deletePVCIfNeeded deletes a single PVC if it meets the deletion criteria
func (vmro *vmrestoreOperator) deletePVCIfNeeded(
	vm *kubevirtv1.VirtualMachine,
	vmr *harvesterv1.VirtualMachineRestore,
	...
	// Only delete PVCs that are in the deleted volumes list
	if !containsVolume(deletedVolumes, pvcName) {
		return nil
	}

	// Try to get the PVC
	pvc, err := vmro.pvcCache.Get(vm.Namespace, pvcName)
	if err != nil {
		return vmro.handlePVCGetError(vm.Namespace, pvcName, err, errs)
	}

	// Try to delete the PVC
	return vmro.deletePVC(pvc, vmr, deletedCount, errs)
}

// handlePVCGetError handles errors when getting a PVC
func (vmro *vmrestoreOperator) handlePVCGetError(namespace, pvcName string, err error, errs *[]error) error {
	if apierrors.IsNotFound(err) {
		logrus.Debugf("PVC %s/%s already deleted or not found", namespace, pvcName)
		return nil
	}

	*errs = append(*errs, fmt.Errorf("failed to get PVC %s/%s: %w", namespace, pvcName, err))
	return err
}

// deletePVC deletes a PVC and tracks the result
func (vmro *vmrestoreOperator) deletePVC(
	...
	err := vmro.pvcClient.Delete(pvc.Namespace, pvc.Name, &metav1.DeleteOptions{})
	if err == nil {
		*deletedCount++
		return nil
	}
	...
	if apierrors.IsNotFound(err) {
		logrus.Debugf("PVC %s/%s already deleted", pvc.Namespace, pvc.Name)
		return nil
	}

	*errs = append(*errs, fmt.Errorf("failed to delete PVC %s/%s: %w", pvc.Namespace, pvc.Name, err))
	return err
}

// containsVolume checks if a volume name is in the list of deleted volumes
func containsVolume(volumes []string, volumeName string) bool {
