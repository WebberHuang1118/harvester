				...
				Spec: corev1.PodSpec{
					...
					Containers: []corev1.Container{{
						...
						Args: []string{fmt.Sprintf(
							...
							"set -eo pipefail; "+
								"restic -q dump --tag=%s,%s latest /%s | /usr/bin/harvester io-mode -device /dev/%s -mode=write",
...
// refreshProgressFromLogs reads the latest IO_PROGRESS percent from the named
// pod and persists it on the VolumeRestore, capped at 99 (100 is reserved for
// syncFromJob's success path so it stays a reliable "fully done" marker).
// Best-effort — on any failure the stored value is left unchanged. Returns
// the resulting progress for the caller.
